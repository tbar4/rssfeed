...
impl <'a>RssClient<'a> {
    pub async fn new(url: &'a str) -> Self {
        Self {
            url,
            channel: read_rss(url).await.unwrap(),
            links: read_links_from_rss(channel).await.unwrap(),
            html_response: join_content_into_html(get_content_from_feed(links)).await.unwrap(),
            md_response: html_to_md(html_response),
        }
    }
}
...
async fn get_content_from_feed(links: Vec<String>) -> Result<Vec<String>> {
...
async fn join_content_into_html(content: Vec<String>) -> Result<Vec<String>> {
    let mut join_handle: Vec<JoinHandle<String>> = vec![];
